...
class Generator(object):
    def __init__(self, input_length, sequence_length, num_classes, num_vocabulary,
        ...
        with tf.variable_scope('place_holder'):
            ...
            self.inputs = tf.placeholder(tf.int32, shape=[self.batch_size, self.input_length])
        ...
        with tf.variable_scope('Worker'):
            ...
            with tf.variable_scope("encoder"):
                self.encoder_W = tf.Variable(tf.random_normal([self.emb_dim * self.input_length, self.hidden_dim]),
                                             name="W")
                self.encoder_b = tf.Variable(tf.zeros(self.hidden_dim), name='b')
            self.worker_params.extend([self.encoder_W, self.encoder_b])
            ...
            self.h0_worker = tf.stack([densed, densed])
    ...
    def pretrain_step(self, sess, inputs, x, dropout_keep_prob):
        outputs = sess.run([self.pretrain_worker_updates, self.pretrain_worker_loss, self.pretrain_manager_updates,
                           ...
                           feed_dict={self.x: x, self.drop_out: dropout_keep_prob, self.inputs: inputs})
        return outputs

    def get_nll(self, sess, x, inputs):
        dropout_keep_prob = 1
        outputs = sess.run([self.pretrain_worker_updates, self.pretrain_worker_loss, self.pretrain_manager_updates,
                            self.pretrain_goal_loss],
                           feed_dict={self.x: x, self.drop_out: dropout_keep_prob, self.inputs: inputs})
    ...
    def generate(self, sess, dropout_keep_prob, inputs, train=1):
        outputs = sess.run(self.gen_x, feed_dict={self.drop_out: dropout_keep_prob, self.inputs: inputs,
                                                  self.train: train})
        return outputs
